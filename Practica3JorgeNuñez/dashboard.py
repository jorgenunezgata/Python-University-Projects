...
class Dashboard:
    ...
    def add_player(self, client_name, client_socket, client_address):
        self.client_info['client_names'].append(client_name)
        self.client_info['client_sockets'].append(client_socket)
        self.client_info['client_addresses'].append(client_address)

    def number_of_players(self):
        return len(self.client_info['client_names'])
    def players_names(self):
        return f"{self.client_info['client_names'][0]} and {self.client_info['client_names'][1]} "

    def __eq__(self, other):
        return self.client_info['client_addresses'][0] == other or self.client_info['client_addresses'][1] == other

    def is_full(self):
        return self.number_of_players() == 2

    def creator_name(self):
        return self.client_info['client_names'][0]

    def creator_socket(self):
        return self.client_info['client_sockets'][0]

    def opponent_name(self):
        return self.client_info['client_names'][(self.turn + 1) % 2]

    def opponent_socket(self):
        return self.client_info['client_sockets'][(self.turn + 1) % 2]
